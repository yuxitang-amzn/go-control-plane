...
func verifyResponse(t *testing.T, ch <-chan cache.Response, version string, num int) {
	t.Helper()
	r := <-ch
...
func mustBlock(t *testing.T, w <-chan cache.Response) {
	select {
	case <-w:
...
func TestLinearInitialResources(t *testing.T) {
	...
	w, _ := c.CreateWatch(&cache.Request{ResourceNames: []string{"a"}, TypeUrl: testType})
	verifyResponse(t, w, "0", 1)
	w, _ = c.CreateWatch(&cache.Request{TypeUrl: testType})
...
func TestLinearCornerCases(t *testing.T) {
	...
	w, _ := c.CreateWatch(&cache.Request{TypeUrl: "test"})
	select {
	case _, more := <-w:
		if more {
...
func TestLinearBasic(t *testing.T) {
	...
	w1, _ := c.CreateWatch(&cache.Request{ResourceNames: []string{"a"}, TypeUrl: testType, VersionInfo: "0"})
	mustBlock(t, w1)
	w, _ := c.CreateWatch(&cache.Request{TypeUrl: testType, VersionInfo: "0"})
	mustBlock(t, w)
	...
	w, _ = c.CreateWatch(&cache.Request{ResourceNames: []string{"a"}, TypeUrl: testType, VersionInfo: "0"})
	checkWatchCount(t, c, "a", 0)
	verifyResponse(t, w, "1", 1)
	w, _ = c.CreateWatch(&cache.Request{TypeUrl: testType, VersionInfo: "0"})
	checkWatchCount(t, c, "a", 0)
...
func TestLinearVersionPrefix(t *testing.T) {
	...
	w, _ = c.CreateWatch(&cache.Request{ResourceNames: []string{"a"}, TypeUrl: testType, VersionInfo: "instance1-1"})
	mustBlock(t, w)
...
func TestLinearDeletion(t *testing.T) {
	...
	checkWatchCount(t, c, "b", 0)
	c.DeleteResource("b")
	w, _ = c.CreateWatch(&cache.Request{TypeUrl: testType, VersionInfo: "1"})
...
func TestLinearWatchTwo(t *testing.T) {
	c := NewLinearCache(testType, WithInitialResources(map[string]types.Resource{"a": testResource("a"), "b": testResource("b")}))
	w, _ := c.CreateWatch(&cache.Request{ResourceNames: []string{"a", "b"}, TypeUrl: testType, VersionInfo: "0"})
	mustBlock(t, w)
	w1, _ := c.CreateWatch(&cache.Request{TypeUrl: testType, VersionInfo: "0"})
	mustBlock(t, w1)
	c.UpdateResource("a", testResource("aa"))
...
func TestLinearCancel(t *testing.T) {
	...
	// cancel watch-all
	w, cancel := c.CreateWatch(&cache.Request{TypeUrl: testType, VersionInfo: "1"})
	mustBlock(t, w)
	checkWatchCount(t, c, "a", 1)
	cancel()
	checkWatchCount(t, c, "a", 0)
	...
	// cancel watch for "a"
	w, cancel = c.CreateWatch(&cache.Request{ResourceNames: []string{"a"}, TypeUrl: testType, VersionInfo: "1"})
	mustBlock(t, w)
	checkWatchCount(t, c, "a", 1)
	cancel()
	checkWatchCount(t, c, "a", 0)
	...
	w, cancel = c.CreateWatch(&cache.Request{ResourceNames: []string{"a"}, TypeUrl: testType, VersionInfo: "1"})
	w2, cancel2 := c.CreateWatch(&cache.Request{ResourceNames: []string{"b"}, TypeUrl: testType, VersionInfo: "1"})
	w3, cancel3 := c.CreateWatch(&cache.Request{TypeUrl: testType, VersionInfo: "1"})
	w4, cancel4 := c.CreateWatch(&cache.Request{TypeUrl: testType, VersionInfo: "1"})
	mustBlock(t, w)
	mustBlock(t, w2)
	mustBlock(t, w3)
	mustBlock(t, w4)
		...
		func(i int) {
			t.Run(fmt.Sprintf("worker%d", i), func(t *testing.T) {
				...
				if i%2 == 0 {
					...
					value, _ := c.CreateWatch(&cache.Request{
